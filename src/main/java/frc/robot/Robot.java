...
public class Robot extends TimedRobot {
  ...
  private static final double kUnitsPerRevolution =
    Constants.unitsPerRevolution;
  public Command m_autonomousCommand;
  public RobotContainer m_robotContainer;
  public Timer timer;
  public Timer disabledTimer;
  public static final XboxController _operator = new XboxController(
    Constants.operatorControllerPort
  );
  public static final TalonFX elevator_crude = new TalonFX(
    Constants.elevatorCrudeTalonID
  );
  ...
  public static final DigitalInput bottomlimit = new DigitalInput(
    Constants.bottomLimitSwitchPort
  );
  public static final TalonFX intake_batman = new TalonFX(
    Constants.intakeBatmanTalonID
  );
  public static final TalonFX intake_robin = new TalonFX(
    Constants.intakeRobinTalonID
  );
  public static final PneumaticHub phub = new PneumaticHub(
    Constants.pneumaticHubID
  );
  public static final Solenoid batsolenoid = new Solenoid(
    Constants.pneumaticHubID,
    PneumaticsModuleType.REVPH,
    Constants.batSolenoidID
  );
  public static final Solenoid robsolenoid = new Solenoid(
    Constants.pneumaticHubID,
    PneumaticsModuleType.REVPH,
    Constants.robSolenoidID
  );
  public double _pos = wrist.getSelectedSensorPosition();
  public static final DutyCycleEncoder gearabsolute = new DutyCycleEncoder(
    Constants.gearAbsoluteEncoderPort
  );
  public static final DutyCycleEncoder armabsolute = new DutyCycleEncoder(
    Constants.armAbsoluteEncoderPort
  );
  public static final ColorSensorV3 m_colorSensor = new ColorSensorV3(i2cPort);
  public static final SN_Blinkin SN_Blinkin = new SN_Blinkin(
  ...
  public void robotInit() {
    ...
    elevator_crude.configSelectedFeedbackSensor(
      TalonFXFeedbackDevice.IntegratedSensor,
      0,
      30
    );
    _gearbox.configSelectedFeedbackSensor(
      TalonFXFeedbackDevice.IntegratedSensor,
      0,
      30
    );
    ...
    elevator_crude.setStatusFramePeriod(
      StatusFrameEnhanced.Status_13_Base_PIDF0,
      10,
      Constants.kTimeoutMs
    );
    _gearbox.setStatusFramePeriod(
      StatusFrameEnhanced.Status_13_Base_PIDF0,
      10,
      Constants.kTimeoutMs
    );
  ...
  public void teleopPeriodic() {
    ...
    double targetPositionEncoderUnits =
      targetPositionDegrees / armabsolute.getDistancePerRotation();
    ...
    // wrist.set(TalonFXControlMode.PercentOutput, _operator.getRawAxis(1) / 2);
    // } else {
    // wrist.set(TalonFXControlMode.Position,
    // elevator_crude.getSelectedSensorPosition(0));
    // }
    ...
    if (bottomlimit.get()) {
      if (!(_operator.getRawAxis(5) == 0)) elevator_crude.set(
        TalonFXControlMode.PercentOutput,
        _operator.getRawAxis(5) / 3
      ); else {
